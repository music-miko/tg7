...
// same chat. Kept conservative for the same reason as leave delays: joins
...
func (c *TelegramCalls) JoinAllAssistants(bot *td.Client, chatID int64) []AssistantJoinResult {
	...
	var ordered []indexed
	for index, acc := range c.accounts {
		ordered = append(ordered, indexed{index, acc})
	}
	c.mu.RUnlock()

	results := make([]AssistantJoinResult, 0, len(ordered))
	for i, entry := range ordered {
		if entry.acc == nil || entry.acc.App == nil {
			continue
		}

		me := entry.acc.App.Me()
		err := c.ensureAssistantMember(bot, chatID, entry.acc, entry.index)
