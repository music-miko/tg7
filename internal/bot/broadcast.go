...
`
	...
	// time re-sending to targets already known to be unreachable.
		...
		for _, chatID := range targets {
			...
			if errSend == nil {
				...
				if isGroup && isChatGoneError(errSend) {
					_ = db.Instance.MarkChatsInvalid([]int64{chatID})
					skipped++
					continue
				}
				if !isGroup {
					if blocked, deleted := isUserGoneError(errSend); blocked || deleted {
						if blocked {
							_ = db.Instance.MarkUsersBlocked([]int64{chatID})
						} else {
							_ = db.Instance.MarkUsersDeleted([]int64{chatID})
						}
						skipped++
						continue
					}
