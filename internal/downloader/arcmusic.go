...
// (/youtube/v2/*). It is only used for YouTube search and download - every
// other platform (Spotify, Apple Music, SoundCloud, Deezer, JioSaavn, etc.)
// continues to use the generic apiData client configured via API_URL / API_KEY.
...
const (
	...
	// downloadTimeout used by the generic API_URL platforms is too short
	// for big YouTube tracks/videos.
...
//     resolved inline by the API - no polling needed.
...
// GetUrlTrack uses "id" and an int32 duration in seconds - converted below.
...
func durationToSeconds(d string) int32 {
	parts := strings.Split(d, ":")
	var total int32
	for _, p := range parts {
		n := 0
		_, _ = fmt.Sscanf(p, "%d", &n)
		total = total*60 + int32(n)
	}
	return total
}

// search calls the ArcMusic /youtube/v2/search endpoint and returns results
// as []utils.GetUrlTrack, matching the shape used by the rest of the Go codebase.
func (a *arcMusic) search(query string, limit int) ([]utils.GetUrlTrack, error) {
	if !a.isConfigured() {
	...
	tracks := make([]utils.GetUrlTrack, 0, len(data.Results))
...
// resolve calls the ArcMusic /youtube/v2/download endpoint. The API checks
// its own cache inline before ever touching the scraping pipeline, so a
// cache hit comes back immediately with no job_id (see requestDownload).
//
